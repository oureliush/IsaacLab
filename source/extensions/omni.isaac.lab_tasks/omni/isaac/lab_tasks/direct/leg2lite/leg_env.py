...
from omni.isaac.lab_assets.custom_robots import LEG_CFG 
...
class EventCfg:
  robot_physics_material = EventTerm(
      ...
      mode="startup",
      params={
          "asset_cfg": SceneEntityCfg("Leg_2Lite", body_names=".*"),
  ...
  knee_joint_properties = EventTerm(
      ...
      mode="startup",
      params={
          "asset_cfg": SceneEntityCfg("Leg_2Lite", joint_names="Revolute_3"),
...
#          "lower_limit_distribution_params": (-0.25, 0.25),
  ...
  torque_rando = EventTerm(
      ...
      params={
          "asset_cfg": SceneEntityCfg("Leg_2Lite", joint_names=".*"),
          "torque_limit_distribution_params": (0.8, 1.0),
          "operation": "scale",
          "distribution": "log_uniform",
      },
  )
  ...
  foot_joint_properties = EventTerm(
      ...
      mode="startup",
      params={
          "asset_cfg": SceneEntityCfg("Leg_2Lite", joint_names="Revolute_5"),
#          "upper_limit_distribution_params": (-0.25, 0.25),
#          "lower_limit_distribution_params": (-0.25, 0.25),
...
@configclass 
class Leg2LiteEnvCfg(DirectRLEnvCfg):
    ...
    # robot
    robot_cfg: ArticulationCfg = LEG_CFG.replace(prim_path="/World/envs/env_.*/Robot")
    ...
    knee_dof_name = "Revolute_3"
    foot_dof_name = "Revolute_5"
...
class Leg2LiteEnv(DirectRLEnv):
    cfg: Leg2LiteEnvCfg

    def __init__(self, cfg: Leg2LiteEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.torque_limits = torch.tensor([15.00, 3.98], device=self.device)  # Torque limits for "Knee" and "Foot" actuators
        
        self.motor_torque_limits = torch.tensor([2.50, 1.99], device=self.device)  # Torque limits for "Knee" and "Foot" motors
        self.gearbox_ratio = torch.tensor([6.00, 2.00], device=self.device)
        self.ideal_gearbox_ratio = torch.tensor([6.00, 2.00], device=self.device)
        ...
        base_ratio = torch.tensor([6.0, 2.0], device=self.device)            # nominal [knee, foot]
        ...
        print("Contact sensor body names:", self._contact_sensor.body_names)
    ...
    def _setup_scene(self):
        ...
        self.scene.articulations["Leg_2Lite"] = self.leg
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
      ...
      self.ideal_torques = self.clamped_actions * self.torque_limits
